...
class Frame_Generator():
    def __init__(self, detector, star_power, prop):
    ...
    def generate_frame(self, t, wf, aperture, atmosphere, pupil_wavefront, reset_time = 0.1, integration_time = 0.001):
        ...
        wf3.total_power = self.star_power
        focal_wf = self.prop(wf3)
        self.detector.integrate(focal_wf, integration_time)
        if t % reset_time < 0.0001:
            img = self.detector.read_out(reset = True)
            return wf2.phase, wf3.phase, img
        else:
            img = self.detector.read_out(reset = False)
            return wf2.phase, wf3.phase, img
